...
int main(int argc, char *argv[])
{
    ...
    if (args.iface == NULL)
    {
        LOG_MSG(DEBUG, "Using default interface for the TCP server: \"%s\"",
                        TCP_SERVER_DFLT_IFACE);
    ...
    {
        LOG_MSG(ERR, "The interface \"%s\" apeears to be down or not configured properly",
                      args.iface);
        return -1;
    }
    ...
    if(args.workers <= 0)
    {
        ...
        LOG_MSG(INFO, "Applying default number of workers: \"%d\"",
    ...
    if(args.port < TCP_PORT_MIN || args.port > TCP_PORT_MAX)
    {
        LOG_MSG(WARNING, "Invalid port number (%d)!. Port range [%d;%d]",
                          ...
                          TCP_PORT_MIN,
                          TCP_PORT_MAX);
        LOG_MSG(INFO, "Applying default port number: \"%d\"",
                       TCP_SERVER_DFLT_PORT);
    ...
    service = service_new(BE_SOCKET_PATH);

    /* Create new instance of a server */
    tcp_server = tcp_server_new();
    ret = tcp_server_init(tcp_server,
                          args.iface,
                          args.port,
                          args.workers);
    if (ret == -1)
